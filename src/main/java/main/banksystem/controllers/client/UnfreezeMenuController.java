...
public class UnfreezeMenuController {
    ...
    void initialize() {
        ...
        for(Id id : ids) {
            if (bills.get(id).getBillConditions() == BillConditions.FROZEN) {
        ...
        unfreezeButton.setOnAction(event -> {
            SetBillStatusCommand command = new SetBillStatusCommand(new Id(Long.parseLong(billChoice.getValue())),
                    BillConditions.FROZEN, BillConditions.ACTIVE, new ICommand.Type(false, true));
            CPU cpu = new CPU(status.getUser());
            cpu.heldCommand(command);

            unfreezeButton.getScene().getWindow().hide();
        });
    }
